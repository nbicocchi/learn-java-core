# SOLID Principle
The SOLID principles are a set of five design principles introduced by Robert C. Martin (author of famous book [Clean Code](https://github.com/jnguyen095/clean-code/blob/master/Clean.Code.A.Handbook.of.Agile.Software.Craftsmanship.pdf)).
* They are intended to make designs easier to understand, maintain, reuse, and extend.
* Reduce dependencies, so that developers can change one area of the software without affecting others. 
...
In the context of BankAccount class, managing deposits, withdrawals and interest are reasonable and related responsibilities to account management. 

But *saveBankAccountDetails* and *sendSmsNotification* methods are not related to bank account management’s behavior. Hence, this class is violating SRP.
...
public class BankAccount {
    ...
    public double calculateInterest() {
        // logica per calcolare interessi
        return balance * 0.03; // esempio
    }
}
```
...
public class NotificationService {
    public void sendSmsNotification(BankAccount account, String message) {
        // invia SMS
    }
}
```
...
public class BankAccountService {
    ...
    public BankAccountService(SQLBankAccountRepository repository, NotificationService notificationService) {
    ...
    public void deposit(BankAccount account, double amount) {
        ...
        notificationService.sendSmsNotification(account, "Deposit effettuato: " + amount);
    }

    public void withdraw(BankAccount account, double amount) {
        account.withdraw(amount);
        repository.save(account);
        notificationService.sendSmsNotification(account, "Prelievo effettuato: " + amount);
    }
}
```
...
* `BankAccount` has **a single responsibility**: managing the account.
* Persistence and notifications are **separated** and can be tested independently.
* `BankAccountService` coordinates operations, but does not contain business logic, so it **does not violate SRP**.
...
Here, withdraw method in *SavingsBankAccount* and *CheckingBankAccount* classes working fine. But *LoanBankAccount* have an empty withdraw method, because in loan account withdrawing process not allowed. 

The implementation classes should use only the methods that are required. We should not force the client to use the methods that they do not want to use.
