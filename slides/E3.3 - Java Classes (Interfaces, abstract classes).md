...
We aim to develop a drawing game. As an initial proof of concept, we will focus exclusively on pencil-based drawing, allowing users to create sketches using a variety of colors.

* The **Pencil** class provide methods to draw and change its color.
...
We decide that we’ll create an abstract DrawingTool class, which will be the superclass of the Pencil, Brush and all future classes.
...
By using **abstractions**, object-oriented programming (OOP) provides value: by separating the *what* (abstract behavior) from the *how* (concrete implementations).

* We can express the **capabilities** of all tools without tying the application to a concrete implementation.
...
## Interfaces
An interface can be considered a special kind of class that can't be instantiated. An interface represents an abstract concept by defining only behavior, expressed through method declarations without implementation. 
...
> The **Open/Closed Principle** states that a software module (class, function, or module) should be open for extension but closed for modification. In other words, you should be able to add new behavior without changing existing code, minimizing the risk of introducing bugs in stable code.
>
...
You can declare and implement static attributes and methods in an interface
...
A common drawback of relying directly on interfaces is the potential for **code duplication** across multiple implementing classes. To address this issue, an **abstract class**—declared using the `abstract` keyword—can be used to represent a **generalized concept** that is meant to be **extended by subclasses** rather than instantiated directly, allowing shared behavior to be defined once and reused.
