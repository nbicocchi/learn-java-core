...
We can also use interfaces like `Erasable`, and `Resizable`, which define separate capabilities that drawing tools can have.
...
By using **abstractions**, object-oriented programming (OOP) provides value: by separating the *what* (interfaces, abstract behavior) from the *how* (concrete implementations).
