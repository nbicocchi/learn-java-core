...
In this case, no `String` objects are copied. We are only swapping references to the row arrays. After the swap, `table[0]` refers to `{"Dave", "Eve", "Frank"}` and `table[1]` refers to `{"Alice", "Bob", "Carol"}`.

The operation runs in constant time because it only reassigns references, not the individual elements.
