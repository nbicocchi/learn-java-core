...
#### Dynamic array
The main limitation of a regular array is that its size is fixed at the time of allocation. In other words, if an array is allocated with size (n), it cannot accommodate the ((n+1))‑th element without creating a new array. One possible workaround is to allocate a large array in advance, but this can lead to **significant memory waste**.

This limitation is addressed by the concept of a [dynamic array](https://en.wikipedia.org/wiki/Dynamic_array) (**~O(n)**), which allows the array to **grow dynamically** as needed. When the capacity is exceeded, a new, larger array is allocated and the existing elements are copied over, enabling flexible resizing while maintaining efficient access and iteration.

![](images/collections-resizable-array.avif)

#### Linked list
A [linked list](https://en.wikipedia.org/wiki/Linked_list) (**~O(n)**) is a linear collection of data elements whose order is determined not by their physical placement in memory, but by **links** connecting each element to the next. It consists of a sequence of **nodes**, where each node typically contains **data** and a reference (or link) to the **next node** in the sequence.

This structure allows for **efficient insertion and removal** of elements at any position during iteration. More advanced variants, such as **doubly linked lists** or **circular linked lists**, add additional links to enable more flexible and efficient modifications at arbitrary positions.

A key drawback of linked lists is that **data access is linear**, meaning that accessing a specific node requires traversing all preceding nodes. As a result, **random access is not feasible**, and access times increase linearly as the list grows. Additionally, linked lists tend to have **poorer cache locality** compared to arrays, which can lead to slower overall performance for access-heavy operations.

![](images/collections-linked-list.avif)

#### Binary tree
A [tree](https://en.wikipedia.org/wiki/Self-balancing_binary_search_tree) is a hierarchical data structure used to organize and store data efficiently. It consists of **nodes** connected by **edges**, with a single **root node** at the top. Each node can have **child nodes**, forming branches, while nodes without children are called **leaves**.

**Balanced trees** are trees in which the height of subtrees is kept approximately equal, ensuring that operations such as insertion, deletion, and lookup remain efficient. Balanced trees, such as **AVL trees** or **Red-Black trees**, guarantee **logarithmic-time search**, making them particularly suitable for applications requiring fast and predictable data retrieval, like databases and indexing structures.

Trees are widely used in applications such as **file systems, databases, search algorithms, and organizational structures**.

![](images/collections-balanced-tree.avif)

#### Hash table
A [hash table](https://en.wikipedia.org/wiki/Hash_table) (**~O(1)**) is a data structure that implements an **associative array** or **dictionary**, mapping **keys** to **values**. It relies on a **hash function** to compute an index (also called a **hash code**) into an array of **buckets** or **slots**, where the corresponding value is stored.

During a lookup operation, the key is hashed, and the resulting index indicates the location of the desired value, enabling **fast insertion, deletion, and retrieval** on average.
...
The **Iterable** interface (java.lang.Iterable) is the root interface of the Java collection framework. 

**Iterable objects (objects implementing the iterable interface) can be used within for-each loops**
...
```java
public static void main(String[] args) {
  Set<String> stringSet = Set.of("Nicola", "Denise", "Agata", "Marzia");
...
**Queue**
A collection designed to hold elements prior to processing, typically following a first-in, first-out (FIFO) order. In addition to the basic operations defined by the `Collection` interface, a queue provides specific methods for insertion, removal, and inspection of elements. Each of these operations is available in two variants: one that throws an exception if the operation fails, and another that returns a special value (`null` or `false`, depending on the method).

**Deque**
A collection that supports insertion and removal of elements at both ends. The term *deque* stands for “double-ended queue” and is commonly pronounced “deck.” Most implementations do not impose fixed size limits, although the interface allows for both bounded (capacity-restricted) and unbounded implementations.
