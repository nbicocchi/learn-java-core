...
Each method that returns a stream produces a stream of values each of which is chosen in the same manner as for a method that returns a single pseudorandomly chosen value. For example, if `rnd` implements RandomGenerator, then the method call `rnd.ints(100)` returns a stream of 100 int values. These are not necessarily the exact same values that would have been returned if instead `rnd.nextInt()` had been called 100 times; all that is guaranteed is that each value in the stream is chosen in a similar pseudorandom manner from the same range.
...
public class RandomDemo {
    ...
    public static void main(String[] args) {
        ...
        IntStream i4 = rnd.ints(10);    //stream of 10 pseudorandom int values
        LongStream l4 = rnd.longs(10);    //stream of 10 pseudorandom long values
        DoubleStream d4 = rnd.doubles(10);    //stream of 10 pseudorandom double values
        // each of these functions can be called with a variable number of parameters: zero (then it returns an indefinite number of pseudorandom values ​​of that type), one (streamSize), two (randomNumberOrigin, randomNumberBound), three (streamSize, randomNumberOrigin, randomNumberBound)
