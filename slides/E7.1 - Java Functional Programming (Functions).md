...
public static List<Integer> filterEven(List<Integer> numbers) {
    ...
    for (Integer n : numbers) {
        if (n % 2 == 0) {   // behaviour is hardcoded
            result.add(n);
        }
    }
    return result;
...
public static List<Integer> filter(List<Integer> numbers, Predicate<Integer> predicate) {
    List<Integer> result = new ArrayList<>();
    for (Integer n : numbers) {
        if (predicate.test(n)) {  // behaviour is a parameter
            result.add(n);
        }
    }
    return result;
...
public static void main(String[] args) {
    ...
    // single operation
    Function<Integer, Integer> adder1 = new Function<Integer, Integer>() {
        @Override
    ...
    // multiple statements
    Function<Integer, Integer> adder5 = new Function<Integer, Integer>() {
        @Override
        public Integer apply(Integer x) {
            x += 2;
            x += 3;
...
Some logging methods support **lazy evaluation** by accepting a `Supplier<String>`. The `Supplier` is called **only if the log level requires it**, so the message is computed lazily.


```java
public static testLog() {
    Logger logger = Logger.getLogger("MyLogger");
    logger.info("This message is computed immediately");
    logger.info(() -> "This message is computed lazily");
}
```
