...
**All fields are `protected`, which means they are always visible to subclasses, even if located in a different package**.
...
In general, **upcast** is useful when working with collections containing objects of different types belonging to the same hierarchy.
...
**Explanation:** `p1` and `p2` have the same name, but `equals()` in `Object` only checks if `p1` and `p2` point to the **same object in memory**. Without overriding `equals()`, objects with identical fields will not be considered equal.
...
Java Object `hashCode()` is a method that returns an integer value (the hash code) representing the object. It is used primarily in hash-based collections like `HashMap`, `HashSet`. The general contract of `hashCode()` is closely related to `equals()`:
...
class Person {
    ...
    @Override
    public int hashCode() {
      return Objects.hash(name); // Hash code based on the name field
    }
}
...
* `equals()` says that **p1 and p2 are equal** (same `id`)
* `hashCode()` generates **different hash values** because it also uses `name`
* `HashSet` first uses **hashCode()** to select a bucket
* Since the objects fall into **different buckets**, `equals()` is never checked
...
**Polymorphism** is a fundamental concept in object-oriented programming that allows objects to be treated as instances of their parent class while behaving according to their actual subclass type. 

It enables flexibility and code reuse by allowing the same interface to represent different underlying forms. Polymorphism can be:
* **static (compile-time)**, achieved through method overloading
* **dynamic (run-time)**, achieved through method overriding.
...
**Flexibility & Extensibility:** **Tell, Don’t Ask** principle, which suggests that objects should be **told what to do rather than asked for their internal state and then manipulated externally**. Instead of checking an object’s type or state and deciding what action to perform, the program simply calls a method and lets the object determine the correct behavior.
...
```java
// This approach is not sustainable because it centralizes all decision logic in a single `switch` statement, forcing the programmer to know *in advance* all possible types of `Mammal`. Every time a new type is introduced (e.g., `"dog"`, `"lion"`, etc.), this code must be modified.
public static void main(String[] args) {
    ...
    for (Mammal m : mammals) {
        switch (m.type) {
            ...
            case "cat" -> System.out.println("meeow");
...
**Abstraction & Decoupling:** Polymorphism encourages developers to write code that depends on **abstract types** (superclasses or interfaces) rather than concrete implementations. For instance, a `List<Mammal>` can store objects of different subclasses such as `Cat`, `Human`, or `Dog`, while the program interacts with them through the common `Mammal` abstraction. This reduces coupling between components, making systems **more modular, easier to maintain, and easier to test**, since implementations can be replaced or mocked without affecting the rest of the code.

```java
/***
 * This does not depend on any specific mammal
 * @param mammal to speak
 */
