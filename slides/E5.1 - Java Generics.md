...
Consider a generic method that finds the average of the values in an ArrayList of objects. How can you compute averages when you know nothing about the element type? You need to have a mechanism for measuring the elements.
...
```java
public static double average(ArrayList<? extends Measurable> objects) {
    if (objects.size() == 0) { 
        return 0.0; 
    } 
    double sum = 0;
...
Again, this compiles cleanly and works fine if the element type of the destination collection exactly matches that of the stack. But again, it doesn’t seem entirely satisfactory when using subtypes.

```java
public static void main(String[] args) {
    Stack<Integer> numberStack = new Stack<>();
    numberStack.pushAll(new ArrayList<>(List.of(1,2,3)));
    
    // Ok
    numberStack.popAll(new ArrayList<Integer>());

    // Compile-time error! List<Integer> is not a subtype of List<Number>!
    numberStack.popAll(new ArrayList<Number>());
}
```

The type of the input parameter to *popAll()* should not be *Collection of E* but *Collection of some supertype of E* (where
supertype is defined such that E is a supertype of itself). Again, there is a wildcard type that means precisely that: *Collection<? super E>*. 
...
The same process is applied to generic methods. In the example below you can also see how bounded types (i.e., Measurable is the upper bound of generic type E) are erased: 
