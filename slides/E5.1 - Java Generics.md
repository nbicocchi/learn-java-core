...
Using the alternative syntax, we do not have access to the *T* type. Instead, we can use the defined upper bound: *Measurable*.
...
### Which syntax?
Use wildcards whenever constraints between either parameters or return values are absent:
...
Use <T> whenever constraints are present:
...
Given the absence of constraints, the rightmost approach for the above case is:
