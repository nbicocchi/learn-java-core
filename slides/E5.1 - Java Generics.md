...
Since version 5, Java has supported generic programming that introduces **abstraction over types**. Generic methods and classes can handle different types in the same way.
...
public static void main(String[] args) {
    ...
    int len2 = length(stringArray); // pass an array of Integers
    // ...
}
```
...
Let's now consider an example that will reveal another aspect of generics. Imagine that we have a generic `Storage<T>` class that can contain objects of any class. 
...
We can put any type of object inside `Storage<T>`. However, there are some situations in which we want to restrict these objects. We can say, for example, that the storage has to be able to contain only books. Then we can implement our limitation by adding `<T extends Books>`:
...
Storage<Computers> storage3 = new Storage<>(); // compile-time error
```
...
Now consider the task of finding the minimum in an ArrayList. We can return the element with the smallest measure. We will use the Comparable interface type (instead of Measurable).
...
For example, String implements Comparable<String>. You can compare strings with other strings, but not with objects of different classes. If the ArrayList has elements of type E, then we want to require that E implements Comparable<E>:
...
As you can see, the type parameters K and V have been replaced by Object. The result is an ordinary class. The same process is applied to generic methods:
