...
- **Final Classes**: If a class is declared with the `final` keyword, it **cannot be extended** and cannot have subclasses.
...
This hierarchy has three levels and five classes overall. All fields are `protected` , which means they are always visible to subclasses, even if located into a different package. Each class also has public getters and setters and an all-arguments constructor.
